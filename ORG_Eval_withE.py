...
def input_file(file_name):
    ...
    def dfs(i):
        ...
        current_neighbors = list(temp[i])
        for j in current_neighbors:
            con = dfs(j)
            if con:
                for k in con:
                    if (i, k) not in precedence_relations:
                        precedence_relations.add((i, k))
                        temp[i].append(k)
        ran[i] = 1
        return temp[i]
...
def solve_new(wcnf, var):
    ...
    print(f"Solving {wcnf_filename} with external MaxSAT solver...")
    try:
...
def solve_MaxSat_SAML3P(n, m, c, Ex_Time, W, precedence_relations, file_name, input_in):
    ...
    if model is not None:
        ...
        write_fancy_table_to_csv(file_name, n, m, c, var2, 
                                 ...
                                 done_time, "Binary", build_time, cal_UB_LB_time, filename="Eval_Binary_2024_withE.csv")
    else:
        print("UNSAT")
        write_fancy_table_to_csv(file_name, n, m, c, var2, 
