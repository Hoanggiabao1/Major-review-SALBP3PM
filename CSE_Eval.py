...
def write_wcnf_with_h_prefix(clauses, soft_clauses, var, filename = "problem_cse_eval.wcnf"):
...
def optimal(X,S,A,n,m,c,sol,solbb,start_time):
    ...
    write_wcnf_with_h_prefix(clauses, soft_clauses, var, filename="problem_cse_eval.wcnf")
    res = solve_maxsat()
    if res is None:
        print("no solution")
    ...
    return bestValue, var_counter, clauses, soft_clauses, "Optimal"
...
def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time_elapsed, filename="CSE_Eval_2025.csv"):
...
def reset(idx):
    global n, m, c, val, cons, sol, solbb, type, filename, W, neighbors, reversed_neighbors, visited, toposort, clauses, time_list, adj, forward, var_map, var_counter, current_instance_id, ran
