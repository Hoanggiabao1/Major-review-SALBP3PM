...
def write_wcnf_with_h_prefix(clauses, soft_clauses, var, filename = "problem_cse_eval_noE.wcnf"):
...
def optimal(X,S,A,n,m,c,sol,solbb,start_time):
    ...
    write_wcnf_with_h_prefix(clauses, soft_clauses, var, filename="problem_cse_eval_noE.wcnf")
    res = solve_maxsat()
    if res is None:
        print("no solution")
...
def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time_elapsed, filename="CSE_Eval_2025_noE.csv"):
...
def main():
    ...
    for idx in range(5, len(file_name1)):
        reset(idx)
        read_input()
        X, A, S = generate_variables(n,m,c)
        val = max(S)
